...
ExternalProject_Add(ep_libiconv
    ...
    CONFIGURE_COMMAND
        ...
        "${BASH_EXECUTABLE}" "${AUTOTOOLS_HELPER_MSYS}" "<SOURCE_DIR>" bash -c "./configure \"--host=${ANDROID_TARGET}\" \"--prefix=${EP_AUTOTOOLS_PREFIX}\" --disable-shared --enable-static --with-pic --enable-extra-encodings && sed -i -e '/^#define gid_t/d' -e '/^#define uid_t/d' -e '/^#define HAVE_LANGINFO_CODESET/d' config.h && sed -i -e '/^#define HAVE_LANGINFO_CODESET/d' libcharset/config.h 2>/dev/null || true && sed -i -e 's/#if HAVE_LANGINFO_CODESET/#if 0 \/\/ patched for Android/' libcharset/lib/localcharset.c 2>/dev/null || sed -i -e 's/#if HAVE_LANGINFO_CODESET/#if 0/' lib/localcharset.c 2>/dev/null || true" 
    BUILD_COMMAND
