...
ExternalProject_Add(ep_libiconv
    ...
    LOG_INSTALL 1
    PATCH_COMMAND
        "${BASH_EXECUTABLE}" -c "for f in libcharset/lib/localcharset.c lib/localcharset.c; do [ -f \"$f\" ] && sed -i -e 's/codeset = nl_langinfo (CODESET)/codeset = \"UTF-8\"/' \"$f\" && sed -i -e 's/#if HAVE_LANGINFO_CODESET/#if 0/' \"$f\" && echo \"patched $f\"; done; for f in srclib/fcntl.c; do [ -f \"$f\" ] && (grep -q 'ifndef F_SETFD' \"$f\" || (echo '#ifndef F_SETFD' && echo '#define F_SETFD 2' && echo '#endif' && cat \"$f\") > \"$f.tmp\" && mv \"$f.tmp\" \"$f\" && echo \"patched $f\"); done; echo patched done"
    CONFIGURE_COMMAND
