...
ExternalProject_Add(ep_libiconv
    ...
    CONFIGURE_COMMAND
        ...
        "${BASH_EXECUTABLE}" "${AUTOTOOLS_HELPER_MSYS}" "<SOURCE_DIR>" bash -c "./configure \"--host=${ANDROID_TARGET}\" \"--prefix=${EP_AUTOTOOLS_PREFIX}\" --disable-shared --enable-static --with-pic --enable-extra-encodings && sed -i -e '/^#define gid_t/d' -e '/^#define uid_t/d' config.h"
    BUILD_COMMAND
        ${CMAKE_COMMAND} -E env "CC=${_cc_msys}" "CFLAGS=--target=${ANDROID_TARGET}${ANDROID_PLATFORM_LEVEL}" "CXX=${_cxx_msys}" "CXXFLAGS=--target=${ANDROID_TARGET}${ANDROID_PLATFORM_LEVEL}" "AR=${_ar_msys}" "LD=${_ld_msys}" "RANLIB=${_ranlib_msys}" "STRIP=${_strip_msys}"
